#!/usr/bin/env bash
# Build E3 3D Stencil abstraction variants from kernels/stencil/.
#
# Usage:
#   ./scripts/build/build_stencil.sh --platform nvidia_rtx5060_laptop
#   ./scripts/build/build_stencil.sh --platform nvidia_rtx5060_laptop --cuda-arch 120 \
#       --kokkos-root /path/to/kokkos/install \
#       --raja-dir    /path/to/raja/build
#   ./scripts/build/build_stencil.sh --platform amd_mi300x --abstraction native
#   ./scripts/build/build_stencil.sh --platform nvidia_rtx5060_laptop --clean

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
SOURCE_DIR="${REPO_ROOT}/kernels/stencil"
...
VENDOR="${PLATFORM%%_*}"    # e.g. "nvidia" from "nvidia_rtx5060_laptop"
...
if [[ "${VENDOR}" != "amd" ]]; then
    if [[ -z "${CUDA_ARCH}" ]]; then
        ...
        if [[ -z "${CUDA_ARCH}" ]]; then
            ...
            echo "[build_stencil] Detected CUDA arch: sm_${CUDA_ARCH}"
        fi
    fi
    ...
    echo "[build_stencil] CUDA compiler: ${CUDA_COMPILER}"
fi
...
build_variant() {
    ...
    echo ""
    echo "[build_stencil] ── ${abstraction} ──────────────────────────────────"
    ...
    echo "[build_stencil] ${abstraction} → ${build_dir}"
}

# ── Abstraction build functions ───────────────────────────────────────────────
build_native_cuda() {
    build_variant "cuda" \
        -DSTENCIL_ENABLE_HIP=OFF \
        -DSTENCIL_ENABLE_SYCL=OFF \
        -DSTENCIL_ENABLE_JULIA=OFF \
        -DSTENCIL_ENABLE_NUMBA=OFF \
...
build_sycl() {
    ...
    if [[ "${VENDOR}" == "nvidia" ]]; then
        ...
        sycl_extra_cmake+=(-DSTENCIL_ENABLE_HIP=OFF)
...
build_julia() {
    ...
    build_variant "julia" \
        -DSTENCIL_ENABLE_JULIA=ON \
        -DSTENCIL_ENABLE_HIP=OFF \
        -DSTENCIL_ENABLE_SYCL=OFF \
        -DSTENCIL_ENABLE_NUMBA=OFF \
        -DCMAKE_DISABLE_FIND_PACKAGE_Kokkos=ON \
        -DCMAKE_DISABLE_FIND_PACKAGE_RAJA=ON
    chmod +x "${BUILD_BASE}/julia_${PLATFORM}/stencil-julia" 2>/dev/null || true
...
build_numba() {
    ...
    build_variant "numba" \
        -DSTENCIL_ENABLE_NUMBA=ON \
        -DSTENCIL_ENABLE_HIP=OFF \
        -DSTENCIL_ENABLE_SYCL=OFF \
        -DSTENCIL_ENABLE_JULIA=OFF
    chmod +x "${BUILD_BASE}/numba_${PLATFORM}/stencil-numba" 2>/dev/null || true
}

# ── Dispatch ──────────────────────────────────────────────────────────────────
echo "[build_stencil] =========================================================="
echo "[build_stencil] Source:    ${SOURCE_DIR}"
echo "[build_stencil] Build dir: ${BUILD_BASE}"
echo "[build_stencil] Platform:  ${PLATFORM}"
if [[ "${VENDOR}" != "amd" ]]; then
    echo "[build_stencil] CUDA arch: sm_${CUDA_ARCH}"
    echo "[build_stencil] CUDA nvcc: ${CUDA_COMPILER}"
fi
echo "[build_stencil] Kokkos:    ${KOKKOS_ROOT:-not found}"
echo "[build_stencil] RAJA:      ${RAJA_DIR:-not found}"
echo "[build_stencil] =========================================================="
...
echo ""
echo "[build_stencil] =========================================================="
echo "[build_stencil] Done.  Built binaries:"
find "${BUILD_BASE}" -maxdepth 2 -name "stencil-*" -executable \
    ! -name "*.sh" 2>/dev/null \
    | sort | sed "s|${BUILD_BASE}/|  |"
echo "[build_stencil] =========================================================="
echo "[build_stencil] Run with:"
echo "  ./scripts/run/run_stencil.sh --platform ${PLATFORM}"
