...
|------|---------------|--------|
| `data/raw/quarantine/stream_raja_nvidia_rtx5060_laptop_20260314.csv` | all 30 rows | Full session thermally throttled |
| `data/raw/stream_native_nvidia_rtx5060_laptop_20260314.csv` | rows with `timestamp LIKE '2026-03-14T16:52%'` (rows 61–90, run_id 1–30 from second append) | GPU throttled at session start |

## What happened

After a prolonged RAJA build session, the RTX 5060 Laptop GPU entered sustained
...
- `stream_raja_nvidia_rtx5060_laptop_20260314.csv` → moved to `quarantine/`.
...
- All 5 abstractions rerun in a single thermally-controlled session on 2026-03-14
  ...
  fresh rerun uses the standard `stream_raja_nvidia_rtx5060_laptop_20260314.csv`.
...
-- Use: stream_raja_nvidia_rtx5060_laptop_20260314.csv (fresh rerun)
```
...
Even with 5-min cooldowns the RTX 5060 Laptop GPU non-deterministically
...
|-------------|-----------|---------|-------|
| native (peak) | `stream_native_nvidia_rtx5060_laptop_20260314.csv` rows 1–60 (ts 00:38) | 00:38 | 350 GB/s — true hardware peak |
| native (sustained) | same file, rows 91–120 (ts ~18:xx) | controlled rerun | 277 GB/s |
| kokkos | `stream_kokkos_nvidia_rtx5060_laptop_20260314.csv` latest 30 rows | controlled rerun | 350 GB/s — boost state (artifact) |
| julia | `stream_julia_nvidia_rtx5060_laptop_20260314.csv` latest 30 rows | controlled rerun | 271 GB/s — sustained |
| numba | `stream_numba_nvidia_rtx5060_laptop_20260314.csv` latest 30 rows | controlled rerun | 350 GB/s — boost state (artifact) |
| raja | `stream_raja_nvidia_rtx5060_laptop_20260314.csv` | controlled rerun | 274 GB/s — sustained, **PPC=0.990** |
